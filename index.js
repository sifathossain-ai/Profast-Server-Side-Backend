    ...
    try {
        ...
        app.get('/rider/parcels', async (req, res) => {
            try {
                ...
                if (pendingParcels.length === 0) {
                    return res.status(404).send({ message: "No pending deliveries found" });
                }
                res.send(pendingParcels);

            } catch (error) {
                console.error("Error fetching pending parcels:", error);
                res.status(500).send({ message: "Failed to fetch pending deliveries", error });
            }
        });
        ...
        app.get("/rider/deliveredParcels", async (req, res) => {
            try {
                ...
                const parcels = await parcelCollection.find(query).sort({ updated_at: -1 }).toArray();

                res.status(200).send(parcels);
            } catch (error) {
                res.status(500).send({ message: "Failed to fetch delivered parcels", error });
            }
        });
        ...
        app.get('/riders/pending', verifyFBToken, verifyAdmin, async (req, res) => {
            try {
                ...
                res.status(500).send({ message: "Failed to fetch pending riders", error });
            }
        });
        ...
        app.patch('/riders/:id', async (req, res) => {
            try {
                ...
                res.status(500).send({ success: false, message: "Failed to update rider status", error });
            }
        });
                    ...
                    .find(
                        {
                            ...
                            delivery_status: { $in: ["not_collected", "assigned", "transit", "delivered"] }
                        }
                    )
                    .sort({ updated_at: -1 })
                    .toArray();
        ...
        app.post('/tracking', async (req, res) => {
            try {
                const { tracking_id, parcel_id, status, massage, update_by = '' } = req.body;
                const tracking = {
